...
CREATE TABLE IF NOT EXISTS messages (
  ...
  scheduled_days      TEXT[]      DEFAULT NULL,  -- null=매일, ['mon','tue',...] = 특정 요일
...
BEGIN
  ...
  -- 오늘 요일 (한국 시간 기준): mon/tue/wed/thu/fri/sat/sun
  v_day := LOWER(TO_CHAR(NOW() AT TIME ZONE 'Asia/Seoul', 'Dy'));
  ...
  WHERE m.tenant_id = v_tenant_id
    AND m.active = TRUE
    AND (m.scheduled_days IS NULL OR v_day = ANY(m.scheduled_days))
    AND (
      m.target_type = 'all'
